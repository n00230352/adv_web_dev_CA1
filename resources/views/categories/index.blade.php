        ...
        <div class="bg-gradient-to-r from-gray-900 to-gray-700 p-6 rounded-lg shadow-lg">
            ...
            <form action="{{ route('categories.search') }}" method="GET" class="mt-4">
                <div class="flex categories-center">
                    <!-- Search input box -->
                    <input type="text" name="search" value="{{ request()->input('search') }}" placeholder="Search categories..."
                            ...
                            <div class="bg-gray-50 border border-gray-300 p-4 rounded-lg shadow-sm flex flex-col h-full">
                                ...
                                @if(auth()->user()->role === 'admin')
                                    <div class="mt-auto space-x-2 flex justify-between">
                                        <a href="{{ route('categories.edit',  $category) }}" class="text-gray-700 border border-gray-400 hover:bg-gray-300 font-bold py-2 px-4 rounded">
                                            Edit
                                        </a>

                                        <!-- Delete form with confirmation -->
                                        <form action="{{ route('categories.destroy',  $category) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this categories?')">
                                            @csrf
                                            @method('DELETE')
